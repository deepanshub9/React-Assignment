...
const styles = {
  root: { padding: "20px" },
  fab: { marginTop: 8, position: "fixed", top: 2, right: 2 },
};
...
const MovieListPage: React.FC = () => {
  ...
  const displayedMovies = movies
    ...
    .filter((m: BaseMovieProps) => (genreId > 0 ? m.genre_ids?.includes(genreId) : true));
  ...
  useEffect(() => {
    fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&include_adult=false&page=1`)
      .then((res) => res.json())
